...
その後、2段階の洞察を書いてください。"""
            ...
            system_prompt = f"""【絶対禁止ルール】
現在時刻は{current_time}である。
この時刻以前の時間帯を指示することは絶対に禁止する。
例）現在{current_time}であれば:
・朝のうちに → 絶対禁止
・午前中に → 絶対禁止
・朝起きたら → 絶対禁止
現在時刻（{current_time}）以降、今夜就寝前までに実行できる行動のみを提示すること。
...
「鑑定予約」と入力してください。🌙"""
            ...
            user_message = f"""現在時刻は{current_time}です。
{current_time}より前の時間帯の行動指針は絶対に含めないでください。
