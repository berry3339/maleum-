...
その後、2段階の洞察を書いてください。"""
            ...
            if current_hour < 12:
                time_rule = (
                    ...
                    "処方箋の時間軸：今日の午後から夕方にかけてを基準に行動を提案すること。\n"
                    "【絶対禁止】深夜・就寝前・今夜 という表現は使わないこと。\n"
                    "今日の午後から夕方の間に実行できる行動のみを提示すること。\n"
                    "このルールに違反した場合、回答全体が無効となる。"
                )
                time_instruction = f"現在{current_time}（午前）。今日の午後から夕方の行動のみ提案してください。深夜・就寝前・夜の言及は禁止。"
            elif current_hour < 18:
                time_rule = (
                    f"【最優先ルール — 時間帯：午後（現在{current_time}）】\n"
                    "処方箋の時間軸：今夜・夕方から夜を基準に行動を提案すること。\n"
                    "【絶対禁止】朝・午前・朝のうちに・午前中に という表現は絶対禁止。\n"
                    "夕方から今夜就寝前までに実行できる行動のみを提示すること。\n"
                    "このルールに違反した場合、回答全体が無効となる。"
                )
                time_instruction = f"現在{current_time}（午後）。夕方から今夜の行動のみ提案してください。朝・午前の言及は禁止。"
            else:
                time_rule = (
                    f"【最優先ルール — 時間帯：夜（現在{current_time}）】\n"
                    "処方箋の時間軸：就寝前・明日の朝を基準に行動を提案すること。\n"
                    "【絶対禁止】今日の昼・午後 への言及は禁止。すでに過ぎた時間帯の提案をしないこと。\n"
                    "今夜就寝前か、明日の朝に実行できる行動のみを提示すること。\n"
                    "このルールに違反した場合、回答全体が無効となる。"
                )
                time_instruction = f"現在{current_time}（夜）。就寝前か明日の朝の行動のみ提案してください。今日の昼・午後の言及は禁止。"
