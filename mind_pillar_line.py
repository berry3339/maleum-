...
def build_mystery_kataomoi_card():
    """片思い 결제 전 미스터리 카드 (핑크 테마)"""
    return {
        "type": "bubble",
        "body": {
            "type": "box",
            "layout": "vertical",
            "backgroundColor": "#3d1a3a",
            "paddingAll": "20px",
            "contents": [
                {"type": "text", "text": "好きな人との両思いの可能性",
                 "size": "sm", "color": "#FFB7C5", "align": "center"},
                {"type": "text", "text": "??%",
                 "size": "3xl", "weight": "bold", "color": "#FF69B4", "align": "center"},
                {"type": "separator", "margin": "lg"},
                {"type": "text",
                ...
                {"type": "text",
                 "text": "━━━━━━━━━━━━",
                 "size": "xxs", "color": "#775555", "align": "center", "margin": "md"},
                {"type": "text", "text": "🔒 おしはらいの後に公開されます",
                 "size": "xxs", "color": "#FFB7C5", "align": "center", "margin": "md"}
            ]
        }
    }


def build_kataomoi_omamori_card():
    """片思い 부적 카드 (핑크 테마)"""
    return {
        "type": "bubble",
        "body": {
            ...
            "contents": [
                ...
                {"type": "text", "text": "このカードをスクショして\nロック画面に設定してね",
                 "size": "xs", "color": "#CCCCCC",
                 "align": "center", "wrap": True, "margin": "lg"},
                {"type": "text", "text": "毎朝起きたとき\nあの人の笑顔を\n心の中で思い描いてみて🌸",
                 "size": "sm", "color": "#FFFFFF",
                 "align": "center", "wrap": True, "margin": "lg"},
                {"type": "separator", "margin": "lg",
                 "color": "#FF69B450"},
                {"type": "text", "text": "あなたの気持ちが\nそっとあの人に届いていくよ✨",
                 "size": "xs", "color": "#FFB7C5",
                 "align": "center", "wrap": True, "margin": "md"},
                {"type": "text", "text": "🔮 マルム｜恋の処方箋",
                 "size": "xxs", "color": "#888888",
                 "align": "center", "margin": "lg"}
            ]
        }
...
def build_kataomoi_payment_ticket_card(price, payment_url):
    """片思い 전용 결제 티켓 카드"""
    return {
        "type": "bubble",
        "body": {
            ...
            "paddingAll": "20px",
            "contents": [
                {"type": "text", "text": "💌 好きな人との運命の処方せん",
                 "size": "sm", "color": "#FF69B4",
                 "align": "center", "weight": "bold", "wrap": True},
                {"type": "separator", "margin": "lg", "color": "#FF69B450"},
                {"type": "text",
                 "text": "あなただけの処方せんが\n準備できてるよ✨",
                 "size": "xs", "color": "#FFFFFF",
                 "align": "center", "wrap": True, "margin": "lg"},
                ...
                {"type": "text", "text": "届くもの:",
                 "size": "xs", "color": "#FF69B4",
                 "weight": "bold", "margin": "lg"},
                {"type": "text", "text": "🌸 好きな人の本音とあなたへの印象",
                 "size": "xs", "color": "#FFFFFF", "wrap": True, "margin": "sm"},
                {"type": "text", "text": "🌸 あなたを意識するタイミング",
                 "size": "xs", "color": "#FFFFFF", "wrap": True, "margin": "sm"},
                {"type": "text", "text": "🌸 アプローチのベストタイミング",
                 "size": "xs", "color": "#FFFFFF", "wrap": True, "margin": "sm"},
                {"type": "text", "text": "🌸 絶対やらないでほしいこと",
                 "size": "xs", "color": "#FFFFFF", "wrap": True, "margin": "sm"},
                {"type": "separator", "margin": "lg", "color": "#FF69B430"},
                {"type": "text",
                 "text": "おしはらいの後すぐに届くよ⚡",
...
def build_payment_ticket_card(price, payment_url, code, title="うんめいの処方せん", items=None):
    ...
    if items:
        body_contents.append(
            {"type": "separator", "margin": "md", "color": "#FFD70030"}
        )
        body_contents.append(
            {"type": "text", "text": "届くもの",
             "size": "xxs", "color": "#FFD700",
...
その後、2段階の洞察を書いてください。"""
            ...
            PREVIEW_HITOKOTO = {
                "木": "今日は「始める」より「育てる」ことに意識を向けてみてね🌱",
                "火": "気持ちが乗ってきたら、そのままでいいよ。その熱がエネルギーになるから🔥",
                "土": "誰かのために動いたこと、ちゃんと自分にも返ってくるよ🌿",
                "金": "こだわりを大切にしていい日だよ。自分の感覚を信じてね✨",
                "水": "流れに逆らわなくていい。今日は感じるままに動いてみてね🌊",
            }
            tagline = (
                f"{ohaeng_emoji} {day_yomi}"
                f" — {PREVIEW_TAGLINE.get(saju['ohaeng'], '命のエネルギー。')}"
                f"\n\n🌙 今日のひとこと: {PREVIEW_HITOKOTO.get(saju['ohaeng'], 'あなたのペースで大丈夫だよ✨')}"
            )
            return tagline
...
小学校1年生でも読める文章にすること。
漢字は以下だけ許可:
推し、人、今日、明日、大、小、気、心、午前、午後

それ以外の漢字は全てひらがなで書け。
ただし「午前」「午後」は必ず漢字で書くこと。ごぜん・ごごと書かないこと。
特に以下は絶対禁止:
太陽→たいよう / 関係→つながり
完璧→かんぺき / 役割→ポジション
成長→のびてる / 沈黙→だんまり
...
【五行の表現ルール — 最重要】
五行の専門用語（金属、火、木、土、水）をそのまま使わないで。
日常的なたとえに変換して。
例: "太陽と宝石みたいに輝かせる関係" など
...
【ケミ・ポジション表現ルール — 最重要】
「二人のケミ」「あなたのポジション」「推しのポジション」を書くとき:
- 抽象的な比喩（太陽、宝石、光、エネルギー等）は使わないこと
- 代わりに、ファンが日常で共感できる具体的な行動や場面で書くこと
- 例:
  ✕ 「太陽が宝石を輝かせる関係」
  ○ 「ライブで誰よりも声出して応援するタイプ。推しがあなたのうちわ見つけたら絶対笑う」
  ✕ 「あつい気持ちの応援リーダー」
  ○ 「新曲が出たら誰よりも先に歌詞を覚えて、友達に布教しちゃうタイプ」
  ✕ 「完璧を目指すがんばり屋さん」
  ○ 「ステージでは完璧だけど、オフショットで見せるギャップに弱いかも」
ファンが「わかるー！」と思える具体的な「あるある」シーンで書くこと。
...
【ふたりのつながり】
1行: ファンあるあるの具体的なシーンで書くこと（「わかるー！」と思える内容で）
...
【推しのポジション 追加ルール】
「💖 推しのポジション」の最後に必ず以下の1行を追加すること:
今日の日付(日)で選ぶ（毎日変えること）:
- 日付 1〜10: 「あなたの応援、推しはちゃんと届いてるよ💖」
- 日付 11〜20: 「推しはあなたのこと、ファンの中でも特別に感じてるかも🌙」
- 日付 21〜31: 「あなたが応援してくれてること、推しは気づいてるよ✨」

必須形式（각 항목 1行ずつ）:
✨ 二人のケミ: [이모지 + ファンあるある具体的シーン1行]
🔮 あなたのポジション: [한 줄]
💖 推しのポジション: [내용 1행＋「推しが気づいてる」メッセージ1行]
🎯 今日の推し活ミッション: [구체적 행동 1개]
⚠️ 今日だけ気をつけて: 以下の10個から今日の日付に合わせて1つだけ選んで出力すること（そのままコピーして使うこと。改変禁止）:
1. 推しが疲れてるかも。そっと見守る優しさも大事だよ🌙
2. 今日は推しに関するネガティブな投稿はスルーしてね🌙
3. 他のファンと比べないで。あなたの応援は特別だから✨
4. 推しのために関連しすぎないでね。あなた自身の時間も大切🌙
5. 今日は推しの過去の投稿を見返すと、いいことあるかも✨
6. 推しも人間だから、完璧じゃない日もあるよ。そんな日も好きでいてあげてね🌙
7. 今日はSNSを少し関連れて、推しとの思い出をゆっくり思い返してみて✨
8. 他の推しのファンに優しくしてあげてね。巡り巡ってあなたに返ってくるよ🌙
9. 今日は推しの曲を聴きながら関連んでみて。気持ちがリセットされるよ✨
10. 推しのことを考えすぎて関連くなってない？深呼吸して、笑顔でいることがいちばんの応援だよ🌙
...
✨ 二人のケミ: 1行のみ
🔮 あなたのポジション: 1行のみ
💖 推しのポジション: 2行（内容1行＋「推しが気づいてる」メッセージ1行）
🎯 今日のミッション: 1行のみ
⚠️ 気をつけて: 1行のみ
📸 シンクロ率: 1行のみ
...
🌙 あの人の本音: [3〜4줄. 핵심만]
🔮 あの人の性格、3つのキーワード:
・[恋愛スタイル — 一途/自由/慎重/情熱的/奥手など]
・[コミュニケーション — 素直/不器用/プライド高い/甘え下手など]
・[今の心理状態 — SNS気になる/連絡したい/距離を置きたいなど]
毎回異なる組み合わせを生年月日と今日の日付に合わせて出力すること。同じ組み合わせを繰り返さないこと。
...
× がんばれる状態 → 代わりに: まだ大丈夫
× ふわっとした日常のひとこと → 代わりに: 「最近どう？」くらいの軽さ
× お守り・スクショ・ロック画面・3回呼んで 等のお守り関連の内容は絶対に出力しないこと
...
💜 あの人との復縁の可能性: ○○%
（85〜95%の間でランダムに設定。必ず最初の行に表示すること）
...
上記の構成でフルレポートを書いてください。"""
    ...
    def get_kataomoi(self, saju1: dict, saju2: dict, partner_name: str = None, mode: str = 'preview') -> str:
        today = datetime.now(ZoneInfo("Asia/Tokyo")).strftime('%Y年%m月%d日')
        ...
        partner_label = partner_name if partner_name else "好きな人"

        if mode == 'preview':
            system_prompt = """【最重要 — 違反したら全てやり直し】
四柱推命の用語は1つも使うな。
丙戌、乙巳、日柱、気質、相生、相剋、火の人、土の人は全て禁止。

代わりに普通の言葉で性格を説明すること。
小学校3年生が読めない漢字はひらがなで書くこと。ただし「午前」「午後」は必ず漢字で書くこと。ごぜん・ごごと書かないこと。
マークダウン禁止。#や**は絶対使わない。
...
🔮 好きな人の性格、3つのキーワード:
・[恋愛スタイル — 一途/自由/慎重/情熱的/奥手など]
・[コミュニケーション — 素直/不器用/シャイ/甘え下手など]
・[あなたへの印象 — 気になってる/意識してる/まだ友達感覚など]
毎回異なる組み合わせを生年月日と今日の日付に合わせて出力すること。同じ組み合わせを繰り返さないこと。
"""
            user_message = f"""今日の日付: {today}
...
上記の構成でプレビューを書いてください。"""
            ...
            system_prompt = """【最重要 — 違反したら全てやり直し】
四柱推命の用語は1つも使うな。全て日常言葉で。
小学校3年生が読めない漢字はひらがなで書くこと。ただし「午前」「午後」は必ず漢字で書くこと。ごぜん・ごごと書かないこと。
マークダウン禁止。#や**は絶対使わない。

【片思いフルレポートのトーン】
恋愛相談に乗ってくれる親友のトーンで。
10代〜30代の女性向け。ドキドキ感を大切に。
...
🌸 好きな人の「恋愛スタイル」
（相手の日柱から読む恋愛傾向。どんな人を好きになるか）

💘 あなたを意識するタイミング
（具体的なシチュエーション1〜2つ）

🎯 アプローチのベストタイミング
（今週〜来週のエネルギーが高まる曜日/時間帯。確定日付ではなく）

⚠️ 絶対やらないでほしいこと
（具体的な行動を1つ挙げ、なぜダメか1文、代わりにどうすればいいか1文。合計2〜3文）
