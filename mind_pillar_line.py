...
その後、2段階の洞察を書いてください。"""
            ...
            pillar_header = (
                f"【あなたの本質：日柱】\n"
                f"{ohaeng_emoji} {saju['day_pillar']}（{day_yomi}）／ {saju['ohaeng']}のエネルギー\n"
                f"（無料版より深く：このエネルギーを自然物で詩的に描写し、現在の大運の流れとの関係まで3〜4行で書くこと）"
            )

            system_prompt = f"""あなたは数十年のキャリアを持つ命理学のマスターです。
...
・現在時刻は{current_time}です。この時刻以前の時間帯（朝・午前など）への言及は絶対禁止。現在時刻から就寝前までに実行可能な行動のみ提示すること。例）現在午後3時であれば、朝・午前に関するアドバイスは完全に削除する。

必ず以下の構成で答えること:

冒頭: 【あなたの本質：日柱】ブロック（フォーマット厳守。無料版より深く書くこと）
...
「鑑定予約」と入力してください。🌙"""
            ...
            user_message = f"""今日の日付: {today}
現在時刻: {current_time}
【時間ルール厳守】現在時刻{current_time}以前の時間帯への言及は絶対禁止。現在時刻から就寝前まで可能な行動のみ記載すること。
{birth_note}
