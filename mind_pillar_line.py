...
「14時」「20時」など24時間表記や具体的な時刻は絶対に使わないでください。時間帯は「朝」「午後」「夕方頃」「夜」などの表現のみ使用してください。
必ず以下のフォーマットを正確に守り、[]の部分だけを埋めて答えてください。それ以外の言葉は絶対に追加しないでください。

🌅 今日は[日柱の五行を一言]のエネルギーで、あなたの一日を先読みしました。

今日は[エネルギー状態を一文で具体的に]。
[朝・午後・夕方・夜などの時間帯]以降に動くと、より良い流れに乗れますよ。
...
【厳守ルール】
・先ほどの内容（エネルギーの流れ・今日の課題）は1行も繰り返さないでください。
・時刻は「午後」「夕方」「夜」など範囲表現のみ使用してください。「14時」「20時」など具体的な時刻は禁止です。
・解説や分析は不要です。アクションプランのみ提示してください。
