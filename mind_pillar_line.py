...
【最重要ルール — 必ず守ること】
・命式データは必ず以下の入力値のみを使用すること。入力されていない干支や命式を絶対に創作・推測しないこと。
・不確実な場合は五行（火・水・木・金・土）のみで表現すること。日柱の干支（例：丁巳、甲申など）に言及する際は必ず入力データからのみ引用すること。
...
2. 【今日の課題】
今日この人が最も強く影響を受けているテーマ——恋愛・今あなたが最もエネルギーを注いでいること・人間関係のうちのひとつ——を命式から見極め、
...
・不確実な場合は五行（火・水・木・金・土）のみで表現すること。日柱の干支（例：丁巳、甲申など）に言及する際は必ず入力データからのみ引用すること。
・「仕事」→「今あなたが最もエネルギーを注いでいること」、「職場」「会社」→「あなたの日常の場」、「上司」→「あなたの周囲にいる人」、「同僚」→「あなたの日常を共にする人」と表現すること。
・先ほどの内容（エネルギーの流れ・今日の課題）は1行も繰り返さないこと。
・「14時」「20時」など24時間表記は絶対禁止。時間帯は「午後」「夕方頃」「夜」「朝のうちに」などの範囲表現のみ使用すること。
・解説や分析は不要。アクションプランのみ提示すること。
